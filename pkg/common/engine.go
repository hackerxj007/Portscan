...
func (e *Engine) randomScan() {
	...
	Count = len(iplist) * len(portlist)
	Bar, _ = pterm.DefaultProgressbar.WithTotal(Count).WithTitle("端口扫描进行").Start()
	Progre, progressErr = NewStatsTicker(1, true)
	if progressErr != nil {
		fmt.Println(progressErr)
	}
	Progre.Init(int64(Count))
